- **사설 저장소 인증서(https) 및 사용자 관리**
  인증서 90일짜리 공짜 다운 받아서 쓸 수 있음(추후 알아볼 예정)
  ...
  -out ./certs/server1.crt \
  -days 10000

  openssl req -x509 -new -key ./certs/ca.key \
  -subj "/C=KR/ST=Seoul/L=Gangnam-gu/O=MyCompany/OU=Development Team/CN=example.com/emailAddress=admin@example.com" \
  -out ./certs/server2.crt \
  -days 10000
  ...
  #SSL/TLS 인증서를 발급받기 위한 중간 단계인 인증서 서명 요청(CSR, Certificate Signing
    Request) 파일을 생성 방법
  openssl req -new -key ./certs/domain.key -subj /CN=172.16.31.152 -out ./certs/domain.csr
  ...
  openssl x509 -req -in ./certs/domain.csr -CA ./certs/server2.crt -CAkey ./certs/ca.key -CAcreateserial -out ./certs/domain.crt -days 10000 -extfile extfile.cnf  -extensions v3_req

  #사설 저장소에 로그인할 사용자 계정 및 비밀번호 생성
  #htpasswd 프로그램 설치
  sudo apt install apache2-utils -y

  #계정 생성 (비밀번호입력 할 것: kosa1004)
  htpasswd -c htpasswd kosa

  #생성된 계정과 비밀번호 파일을 certs 폴더로 이동 한다
  mv htpasswd certs

  #certs/nginx 파일 생성
  # 나중에 키와 인증서를 볼륨으로 연결해줘야함
  # 인증된 사용자만 들어올 수 있도록 설정
  vi certs/nginx.conf
  ---
  ...
  # nginx를 certs을 볼륨 설정 하고 myregistry 컨테이너를 네트웩에서
  # registry이름으로 접근할 실행한다
  ...
  docker login https://172.16.31.152
  ...
  docker login https://172.16.31.145

  #docker 이미지 목록을 확인한다
  docker images -a

  # my-images-name:0.0 이미지가 존재하면 172.16.31.145/my-images-name:0.0으로 tag을 변경한다
  docker tag my-images-name:0.0 172.16.31.145/my-images-name:0.0

  #사설 저장소에 이미지를 push 한다
  docker push  172.16.31.145/my-images-name:0.0

  #로컬 저장소 존재하는 이미지를 삭제 한다
  docker rmi  172.16.31.145/my-images-name:0.0

  #docker 이미지 목록을 확인한다 (172.16.31.145/my-images-name:0.0 이미지 삭제된 것 확인)
  docker images -a

  #사설 저장소에 있는 192.168.1.3/my-images-name:0.0 이미지를 pull 해본다
  docker pull  172.16.31.145/my-images-name:0.0

  #docker 이미지 목록을 확인한다 (172.16.31.145/my-images-name:0.0 이미지가 존재하는지 확인)
  docker images -a

  #새로운 계정 만든 후 로그인 해보기
  htpasswd -c htpasswd kosa12`
  cat htpasswd >> ./certs/htpasswd
  ```

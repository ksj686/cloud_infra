...
## 3. CNI 선택 및 단계별 전환 전략

- **초기 프로토타이핑 (Flannel):**
  - **목적:** 신속한 클러스터 구축 및 단순 파드 간 통신 검증.
  - **특징:** 구조의 단순성 및 낮은 리소스 점유율로 초기 인프라 프로비저닝 속도 향상.
- **운영 환경 전환 (Calico):**
  - **전환 사유:** 보안 정책(Network Policy) 준수, BGP 기반 고성능 라우팅 필요성, 엔터프라이즈 보안 감사 대응.
  - **이행 시나리오:** 서비스 확장 단계(Phase 6)에서 노드 초기화(`kubeadm reset`)를 통한 Calico 재구축 수행.
  - **표준화:** 처음부터 Calico 채택이 권장되나, 학습적 목적의 프로토타이핑-운영 격상을 위해 단계별 CNI 전환 프로세스를 프로젝트 표준으로 삼음.

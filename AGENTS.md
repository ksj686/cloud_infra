...
- 파일 변경이 발생한 작업을 완료할 때는 `git status --short`로 변경 파일을 확인하고, 사용자 변경분과 내가 만든 변경분을 구분하여 보고.
- 파일 변경이 발생한 작업을 완료할 때는 변경 목적별로 그룹화한 한국어 커밋 메시지 초안을 자동 제안. 커밋 메시지는 제목과 주요 변경 내역을 포함.
- 커밋이나 푸시는 사용자가 명시적으로 요청한 경우에만 수행.
